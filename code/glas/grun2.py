...
def robot_composition_to_cpp_types(param,team):
	...
	types = [] 
	for robot_type in param.robot_team_composition[team]:
		if team == "a":
...
def run_mcts_batch(param): 
	...
	for state in states:
		...
		mctsresult = mctscpp.search(game, game_state, generator, param.num_nodes)

		if mctsresult.success: 
			action = mctsresult.valuePerAction
			reward = mctsresult.expectedReward
			actions.append(action)
		else: 
			exit('deal with this case')
	...
	print('completed instance {}'.format(param.dataset_fn))
...
def make_labelled_data(sim_result,oa_pairs_by_size):
	...
	actions = sim_result["actions"] # nt x nrobots x naction_per_robot
...
if __name__ == '__main__':
	...
	if gparam.make_raw_data_on:
		...
		if gparam.serial_on:
			for param in params:
				run_mcts_batch(param)
		else:
			ncpu = cpu_count()
			print('ncpu: ', ncpu)
			with Pool(ncpu-1) as p:
				# p.starmap(run_mcts_batch, params)
				p.imap(run_mcts_batch, params)
